...
if (!API_KEY) {
  console.error('ERROR: GOOGLE_API_KEY is not set. Export GOOGLE_API_KEY and retry.');
  process.exit(1);
}
...
function parseJsonLoose(text) {
  ...
  for (let i = 0; i < t.length; i++) {
    ...
    if (ch === '}') { if (depth > 0 && --depth === 0 && start >= 0) { const obj = t.slice(start, i + 1); try { return JSON.parse(obj); } catch {} } }
  }
  return null;
}
...
function slugify(title, fallback, index) {
  const base = String(title || fallback || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || `section-${index+1}`;
...
- No code fences, no extra commentary—JSON only.`;
  ...
  const res = await gen.generateContent({ contents: [{ role: 'user', parts: [{ text: system }] }] });
  const raw = res?.response?.text?.() || '';
  const json = parseJsonLoose(raw);
  if (!json || !Array.isArray(json.subtopics) || !json.subtopics.length) {
    console.error('Model did not return a valid JSON structure. Raw output:\n', clip(raw, 2000));
    process.exit(2);
  }

  const title = String(json.title || 'Research-Based Optimized Learning Methods');
  const subtopics = json.subtopics.slice(0, 14);
  const normalized = subtopics.map((s, idx) => {
    ...
    })).filter(q => q.prompt && q.options.length === 4 && q.answerIndex >= 0 && q.answerIndex < 4);
    return {
  ...
  const out = `// Auto-generated by scripts/generate-example.mjs\n`+
