  ...
  try {
    const row = await prisma.lecture.findUnique({
      ...
      select: {
        ...
        subtopics: { orderBy: { order: 'asc' }, select: { id: true, order: true, title: true, overview: true, explanation: true } },
      },
    });
    ...
    const shorts = await prisma.shortAnswerPrompt.findMany({ where: { lectureId: row.id }, select: { subtopicId: true, prompt: true } });
    const shortBySub: Record<string, string> = Object.fromEntries(shorts.map((p) => [p.subtopicId, p.prompt]));
    const qBySub: Record<string, PreviewQuestion[]> = {};
    for (const q of mcqs) {
      const k = q.subtopicId;
      (qBySub[k] ||= []).push({ prompt: q.prompt, options: (q.options as unknown as string[]) || [] });
    }
    ...
    return { title: row.title, author: row.user?.username || row.user?.name || 'Unknown', subtopics };
  } catch {
    return null;
  }
}
...
export default async function SharedPreviewPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  async function importAction(formData: FormData) {
    ...
    if (!isSignedIn) {
      redirect(`/login?callbackUrl=${encodeURIComponent(`/lectures/share/${token}`)}`);
    }
  ...
  return <SharePreviewView token={token} data={data} isSignedIn={isSignedIn} onImport={importAction} />;
}
