  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/40 p-4">
          ...
          <div className="mt-2 text-2xl font-semibold">
            {formatNumber(data?.summary.requests || 0)}
          </div>
        </div>
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/40 p-4">
          <div className="text-xs tracking-wide text-neutral-400 uppercase">
          ...
          <div className="mt-2 text-2xl font-semibold">
            {formatNumber(data?.summary.tokensInput || 0)}
          </div>
        </div>
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/40 p-4">
          <div className="text-xs tracking-wide text-neutral-400 uppercase">
          ...
          <div className="mt-2 text-2xl font-semibold">
            {formatNumber(data?.summary.tokensOutput || 0)}
          </div>
        </div>
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/40 p-4">
          <div className="text-xs tracking-wide text-neutral-400 uppercase">
        ...
        <div className="grid grid-cols-1 gap-x-5 gap-y-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
          ...
          <div className="flex flex-col">
            ...
            <input
              ...
              value={from}
              onChange={(e) =>
                setMany({ from: e.target.value, range: undefined, page: '1' })
              }
          ...
          <div className="flex flex-col">
            ...
            <input
              ...
              value={to}
              onChange={(e) =>
                setMany({ to: e.target.value, range: undefined, page: '1' })
              }
              className="input h-10"
            />
          </div>
          <div className="flex flex-col">
            ...
            <input
              ...
              value={model}
              onChange={(e) =>
                setMany({ model: e.target.value || undefined, page: '1' })
              }
              className="input h-10"
          ...
          <div className="flex flex-col">
            ...
            <input
              ...
              value={route}
              onChange={(e) =>
                setMany({ route: e.target.value || undefined, page: '1' })
              }
              className="input h-10"
              placeholder="/api/..."
            />
          ...
          <div className="flex flex-col">
            ...
            <select
              ...
              onChange={(e) => setMany({ perPage: e.target.value, page: '1' })}
              className="select"
            >
              {[25, 50, 100, 200].map((n) => (
          ...
          <div className="flex flex-col sm:col-span-2 lg:col-span-3 xl:col-span-4">
            ...
            <input
              ...
              value={q}
              onChange={(e) =>
                setMany({ q: e.target.value || undefined, page: '1' })
              }
              className="input h-10"
              placeholder="search..."
            />
          ...
          <div className="flex min-w-[220px] items-end justify-end gap-2 sm:col-span-2 lg:col-span-1 xl:col-span-2">
            <button
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="text-left text-neutral-400">
              ...
              <th
                className="cursor-pointer px-4 py-3 text-right tabular-nums"
                onClick={() => updateSort('input')}
              >
                Input {sort === 'input' ? (order === 'asc' ? '▲' : '▼') : ''}
              </th>
              <th
                className="cursor-pointer px-4 py-3 text-right tabular-nums"
                onClick={() => updateSort('output')}
              >
                Output {sort === 'output' ? (order === 'asc' ? '▲' : '▼') : ''}
              </th>
              <th
                className="cursor-pointer px-4 py-3 text-right tabular-nums"
                onClick={() => updateSort('total')}
              >
                Total {sort === 'total' ? (order === 'asc' ? '▲' : '▼') : ''}
              </th>
              <th
                className="cursor-pointer px-4 py-3"
                onClick={() => updateSort('last')}
              >
                Last activity{' '}
                {sort === 'last' ? (order === 'asc' ? '▲' : '▼') : ''}
              </th>
              ...
              <tr key={r.userId} className="border-t border-neutral-900">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    {r.image ? (
                      // Admin rows may include legacy avatar hosts outside the optimized image allow-list.
                ...
                <td className="px-4 py-3 text-right tabular-nums">
                  {formatNumber(r.tokensInput)}
                </td>
                <td className="px-4 py-3 text-right tabular-nums">
                  {formatNumber(r.tokensOutput)}
                </td>
      ...
      <div className="flex items-center justify-between pt-2">
        <div className="text-xs text-neutral-400">
          Page {data ? data.page : page} of {data ? data.totalPages : '-'} ·{' '}
          {data ? formatNumber(data.totalUsers) : '-'} users
        </div>
        <div className="flex items-center gap-2">
          <button
            className="btn-ghost"
            disabled={page <= 1 || loading}
            onClick={() => setMany({ page: String(Math.max(1, page - 1)) })}
          >
            Prev
          </button>
          <button
