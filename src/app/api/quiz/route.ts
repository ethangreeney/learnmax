...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `
      You are an expert in creating educational assessments. Based on the following list of subtopics, generate a quiz to test for mastery.
      The output must be a JSON object with a single key, "questions", which is an array of question objects.
      For each subtopic provided, create one multiple-choice question. Each question object must have these keys:
