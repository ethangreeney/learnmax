...
import { Check, ClipboardX, EllipsisVertical, Link as LinkIcon, Loader2, RefreshCw, XCircle } from 'lucide-react';
import useFocusTrap from '@/hooks/useFocusTrap';
import { useShareLesson } from '@/lib/client/use-share-lesson';

export default function YourLessonActions({ lectureId, initialDiscoverable = false }: { lectureId: string; initialDiscoverable?: boolean }) {
  const { state, setState, createOrUpdate, revoke, copy, cancel } = useShareLesson(lectureId);
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        ...
        const next = items[(idx + dir + items.length) % items.length] as HTMLElement;
  ...
  const [toast, setToast] = useState<{ kind: 'success' | 'ready' | null; url: string | null }>({ kind: null, url: null });
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setToast({ kind: null, url: null }), 2600);
    ...
    <div className="relative">
      ...
      {open && (
        ...
        >
          ...
          >
            {!state.created || state.busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <LinkIcon className="h-4 w-4" />}
            {!state.created || state.busy ? 'Preparing share link…' : 'Share link'}
          </button>
      ...
      {settingsOpen && (
        ...
        >
          ...
          >
            ...
            <p className="mt-1 mb-3 text-sm text-neutral-400">Advanced controls for this lesson's share link.</p>
              ...
              <button
                onClick={async () => {
                  if (typeof window !== 'undefined') {
                    const ok = window.confirm('Revoke share link? This breaks any existing link.');
                    if (!ok) return;
                  }
                  await revoke();
            ...
            <div className="rounded-md border border-yellow-700/40 bg-yellow-900/30 p-3 text-sm text-yellow-100 shadow-lg backdrop-blur">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  {state.shareUrl && (
                    <div className="mt-1 truncate text-xs text-yellow-200/70" title={state.shareUrl}>{state.shareUrl}</div>
                  )}
                </div>
              </div>
                  ...
                  <>
                    ...
                    <button
                      onClick={async () => {
                        try {
                          await navigator.clipboard.writeText(state.shareUrl || '');
                          setToast({ kind: 'success', url: state.shareUrl || null });
                        } catch {}
                      }}
